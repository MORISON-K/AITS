...
function IssueSubmission_form() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const payload = {
        title: `${formData.issueCategory} Issue - ${courses.find(c => c.id === parseInt(formData.courseUnit))?.name || 'Course'}`,
        ...
        {currentStep === 3 && (
          <div className="form-step">
            ...
            >
              ...
              {semesters.map(sem => (
                <option key={sem.value} value={sem.value}>{sem.display}</option>
              ))}
            </select>
            <div className="form-navigation">
              <button type="button" onClick={prevStep} className="prev-button">Previous</button>
